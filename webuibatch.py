...
def generate_batch(model, text, melody, duration, topk, topp, temperature, cfg_coef, base_duration,
                   sliding_window_seconds, continue_file, cf_cutoff, sc_text, seed, directory_name,
                   finetuned_on, batch_mode, num_batches, infinite):
    num_batches = int(num_batches)  # Convert num_batches to an integer
    if infinite:
        while True:
            generated_output = generate(model, text, melody, duration, topk, topp, temperature, cfg_coef,
                                        base_duration, sliding_window_seconds, continue_file, cf_cutoff,
                                        sc_text, seed, directory_name, finetuned_on)
    else:
        # Normal non-batch generation logic
        generated_output = generate(model, text, melody, duration, topk, topp, temperature, cfg_coef,
                                    base_duration, sliding_window_seconds, continue_file, cf_cutoff,
                                    sc_text, seed, directory_name, finetuned_on)
        output.value = generated_output
...
with gr.Blocks(analytics_enabled=False) as demo:
    with gr.Tab("Inference"):
        ...
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    batch_mode = gr.Checkbox(label="Batch Mode", value=False, interactive=True)
                    num_batches = gr.Number(label="Number of Batches", value=5, interactive=True)
                    infinite_checkbox = gr.Checkbox(label="Infinite", value=False, interactive=True)
                ...
                submit.click(generate_batch, inputs=[model, text, melody, duration, topk, topp, temperature,
                                     cfg_coef, base_duration, sliding_window_seconds, continue_file,
                                     cf_cutoff, sc_text, seed, directory_name, finetuned_on,
                                     batch_mode, num_batches, infinite_checkbox],
             outputs=[output])
                gr.Examples(
                    fn=generate,
                    examples=[
